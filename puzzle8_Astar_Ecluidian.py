...
def euclidean_distance(state):
    # Calculate the Euclidean distance of the current state to the goal state
    distance = 0
    goal_positions = {str(i): (i // 3, i % 3) for i in range(9)}  # Goal positions for each tile
    
    for i, tile in enumerate(state):
        if tile != '0':  # Skip the blank tile
            current_pos = (i // 3, i % 3) # Current position of the tile  (row , column)
            goal_pos = goal_positions[tile]     # Goal position of the tile according to the previous map
    ...
    while priority_queue:
        ...
        for successor in generate_children(current_state):
            ...
            if successor not in g_costs or successor_g_cost < g_costs[successor]:
                ...
                f_cost = successor_g_cost + euclidean_distance(successor)
...
def solve_puzzle(initial_state):
    ...
    print(f"Time taken by A* algorithm with euclidean distance heuristic: {time_cost_BFS:.4f} seconds")
